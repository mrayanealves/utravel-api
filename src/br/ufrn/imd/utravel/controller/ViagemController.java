...
public class ViagemController {
    ...
    @Consumes("application/json; charset=UTF-8")
    @Produces("application/json; charset=UTF-8")
    @Path("/{id}/adicionar/transporte")
    @Secured
    public Response adicionarTransporte(@PathParam("id") long id, TransporteDTO transporteDTO, 
    										@Context SecurityContext securityContext) {
    	Usuario usuario = usuarioService.buscarUsuarioPorEmail(securityContext.getUserPrincipal().getName());
    	
    	try {
			return Response.ok(viagemService.adicionarTransporte(id, transporteDTO, usuario)).build();
		} catch (ParseException e) {
			e.printStackTrace();
